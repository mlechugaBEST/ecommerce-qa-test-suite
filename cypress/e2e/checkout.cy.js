 ...
 *
 * Nothing here ever submits an order. CheckoutPage has no placeOrder() method, and the order guard
 * in support/e2e.js blocks BigCommerce's two order/payment endpoints outright as a second line of
 * defense.
 *
 * TWO GATE LAYERS, saying two different things:
 ...
 * Only the second is something an operator can fix, which is why they read differently.
 */
describeIfStore(checkout, 'Checkout (through to the payment step)', () => {
  ...
  itIfStore(creds, 'adds a product, signs in, and reaches the payment step', () => {
    consoleErrors = makeConsoleErrorSpy({ ignore: checkout.consoleIgnore || [] });
    ...
    cy.on('window:before:load', consoleErrors.onBeforeLoad);
    ...
    // than removing it.
    cy.task('log', '[checkout.cy.js] 1/6 clearing cart + session');
    ...
    cy.task('log', `[checkout.cy.js] 2/6 product under test: ${pdpUrl}`);
    ...
    cy.task('log', '[checkout.cy.js] 3/6 opening checkout');
    page.visit();
    cy.location('pathname').should('not.include', 'cart.php');
    page.assertCustomerStep();

    // 4/6 — sign in.
    cy.task('log', '[checkout.cy.js] 4/6 signing in');
    ...
    cy.task('log', '[checkout.cy.js] 5a/6 shipping address');
    ...
    cy.task('log', '[checkout.cy.js] 5b/6 shipping method');
